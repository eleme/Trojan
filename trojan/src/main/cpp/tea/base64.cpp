...
char *b64_encode(const char *src, size_t len) {
    int i = 0;
    int j = 0;
    char *enc = NULL;
    size_t size = 0;
...
char *b64_decode_ex(const char *src, size_t len, size_t *decsize) {
    ...
    while (len--) {
        ...
        if (4 == i) {
            ...
            dec = (char *) b64_realloc(dec, size + 3);
            if (dec != NULL) {
                for (i = 0; i < 3; ++i) {
                    dec[size++] = buf[i];
                }
    ...
    if (i > 0) {
        ...
        dec = (char *) b64_realloc(dec, size + (i - 1));
        if (dec != NULL) {
            for (j = 0; (j < i - 1); ++j) {
                dec[size++] = buf[j];
            }
    ...
    // Make sure we have enough space to add '\0' character at end.
    dec = (char *) b64_realloc(dec, size + 1);
    if (dec != NULL) {
        dec[size] = '\0';
    } else {
        return NULL;
